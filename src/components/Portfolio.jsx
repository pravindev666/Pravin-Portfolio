...
import { projectsByCategory, founderProjects } from '../data/projects';
...
const Portfolio = () => {
  ...
  const renderSkillIcon = (name) => {
    ...
    return (
      <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-yellow-500/20 via-red-500/20 to-yellow-500/30 border border-yellow-500/40 flex items-center justify-center shadow-lg mx-auto">
        <span className="text-yellow-400 font-bold text-xs tracking-wider">{initials}</span>
      </div>
    );
  };
              ...
              <div className="relative z-10">
                ...
                <div className="relative">
                  ...
                  <p className="text-lg mb-8 text-gray-100 leading-relaxed p-4 rounded-lg">
                    AI Infrastructure & Cloud Platform Engineer | Solutions Architect specializing in AI-powered systems, multi-agent orchestration, and enterprise cloud infrastructure. Expert in LLM deployment, RAG pipelines, autonomous AI workflows, Kubernetes, Terraform, Docker & Python. Architecting scalable, intelligent platforms that bridge AI innovation with production-grade DevOps, cloud-native design, and enterprise security practices.
                  </p>
                </div>
              </div>
  ...
  {/* Education Section */}
          ...
          <div className="space-y-8">
            {experience.map((exp, index) => (
              <div key={index} className={`${isDarkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-red-100 to-yellow-100'} rounded-2xl p-10 border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} hover:border-red-500 transition-all duration-500 transform hover:scale-105 hover:shadow-2xl hover:shadow-red-500/20 animate-slideInUp`} style={{animationDelay: `${index * 0.1}s`}}>
                ...
                {/* Forward Deployment Experience sub-section */}
                {exp.forwardDeployment && exp.forwardDeployment.length > 0 && (
                  <div className="mt-8">
                    <div className={`border-t ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} pt-6`}>
                      <h4 className="text-xl font-bold mb-4 bg-gradient-to-r from-red-400 to-yellow-400 bg-clip-text text-transparent">Forward Deployment Experience</h4>
                      <ul className={`space-y-3 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        {exp.forwardDeployment.map((point, i) => (
                          <li key={`fd-${i}`} className="flex items-start">
      ...
      <section id="skills" className={`py-24 px-6 sm:px-8 lg:px-12 ${isDarkMode ? 'bg-gray-900/50' : 'bg-gradient-to-br from-red-50 to-yellow-50'}`}>
        <div className="max-w-7xl mx-auto">
          ...
          <p className={`text-center mb-12 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            A comprehensive snapshot of technologies, tools, and competencies
          </p>

          <div className="space-y-16">
            {/* Cloud Platforms (SVG Icon Cards Grid) */}
            {skills.cloud && (
            ...
            {/* AWS Services (collapsible card) */}
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-red-100 to-yellow-100'} rounded-2xl p-6 border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} shadow-md ${isDarkMode ? 'shadow-black/20' : 'shadow-gray-200'} animate-slideInUp`}>
            ...
            {/* Dynamic Card Grids for all skill categories */}
            {Object.entries(categoryTitles).map(([key, label], catIndex) => {
              if (key === 'cloud') return null; // rendered as top Cloud Platforms section
              const categorySkills = skills[key];
              if (!categorySkills || categorySkills.length === 0) return null;

              return (
                <div key={key} className={`${isDarkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-red-100 to-yellow-100'} rounded-2xl p-10 border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} animate-slideInUp`} style={{animationDelay: `${catIndex * 0.04}s`}}>
                  <h3 className="text-2xl font-bold mb-8 bg-gradient-to-r from-red-400 via-yellow-400 to-yellow-300 bg-clip-text text-transparent drop-shadow-[0_0_10px_rgba(251,191,36,0.5)] brightness-110">{label}</h3>
                    ...
                    <AnimatePresence>
                      {categorySkills.map((skill, index) => (
                        <motion.button
                          key={index}
                          onClick={() => {
                            const docsUrl = getDocsUrl(skill.name);
                            if (docsUrl) window.open(docsUrl, '_blank');
                            else setSelectedSkill(skill.name);
                          }}
      ...
      <section id="projects" className="py-24 px-6 sm:px-8 lg:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="space-y-16">
            {['AWS', 'DevOps', 'Development'].map((category) => {
                ...
                <div key={category} className="animate-slideInUp">
                  ...
                  <div className="grid md:grid-cols-2 gap-10">
                    {categoryProjects.map((project, index) => (
                      <div
                        key={index}
                        className={`${isDarkMode ? 'bg-gray-800' : 'bg-blue-100'} rounded-2xl p-10 border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} hover:border-yellow-500 transition-all duration-500 transform hover:scale-105 hover:shadow-2xl hover:shadow-yellow-500/20 flex flex-col justify-between`}
                        ...
                        <div className="flex items-center gap-3 pt-4 border-t border-gray-700/50">
                          {project.githubUrl && (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-xs font-semibold px-4 py-2 rounded-lg bg-gray-700 hover:bg-gray-600 text-yellow-400 hover:text-yellow-300 flex items-center gap-2 border border-gray-600 transition-all"
                            >
                          ...
                          {project.liveUrl && (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-xs font-semibold px-4 py-2 rounded-lg bg-gradient-to-r from-red-500 to-yellow-500 hover:opacity-90 text-white flex items-center gap-2 transition-all shadow-md"
                            >
                              <span>Live App</span>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                            </a>
      ...
      <section id="founder-projects" className={`py-24 px-6 sm:px-8 lg:px-12 ${isDarkMode ? 'bg-gray-900/50' : 'bg-gradient-to-br from-red-50 to-yellow-50'}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-bold text-center mb-4 bg-gradient-to-r from-red-400 via-yellow-400 to-yellow-300 bg-clip-text text-transparent drop-shadow-[0_0_15px_rgba(251,191,36,0.6)] brightness-110">
            Founder Projects
          </h2>
          <p className={`text-center mb-12 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            AI-first SaaS concepts — end-to-end product planning, architecture design & technical leadership
          </p>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {founderProjects.map((project, index) => (
              <div
                key={index}
                className={`${isDarkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-red-100 to-yellow-100'} rounded-2xl p-8 border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} hover:border-yellow-500 transition-all duration-500 transform hover:scale-105 hover:shadow-2xl hover:shadow-yellow-500/20 animate-slideInUp flex flex-col justify-between`}
                style={{animationDelay: `${index * 0.08}s`}}
              >
                <div>
                  <h4 className="text-xl font-bold mb-3 bg-gradient-to-r from-red-400 to-yellow-400 bg-clip-text text-transparent">{project.title}</h4>
                  <p className={`mb-4 text-sm leading-relaxed ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{project.description}</p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tech.map((tech, i) => (
                      <span key={i} className="px-2.5 py-1 bg-yellow-500/15 text-yellow-500 rounded-full text-xs font-medium">
                        {tech}
                      </span>
                    ))}
                  </div>
      ...
      {/* Advertisement */}
      <script async="async" data-cfasync="false" src="//honeywhyvowel.com/3d7774e8d91a7d26dbbad02185f90823/invoke.js"></script>
      <div id="container-3d7774e8d91a7d26dbbad02185f90823"></div>
